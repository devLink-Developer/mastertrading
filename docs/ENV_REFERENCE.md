...
- `SYMBOL_SIDE_HEALTH_GUARD_ENABLED`: block new entries only for the losing symbol+side pair.
- `SYMBOL_SIDE_HEALTH_GUARD_LOOKBACK_HOURS`, `SYMBOL_SIDE_HEALTH_GUARD_MIN_TRADES`
- `SYMBOL_SIDE_HEALTH_GUARD_MIN_PROFIT_FACTOR`, `SYMBOL_SIDE_HEALTH_GUARD_MIN_EXPECTANCY_PCT`
- `SYMBOL_SIDE_HEALTH_GUARD_EXEMPT_SYMBOLS`, `SYMBOL_SIDE_HEALTH_GUARD_RESET_AT`
- `ENTRY_QUALITY_PROFILE_ENABLED`: fail-closed live allowlist for a deliberately narrow entry profile.
...
- `EUDY_EXPLORATION_MIN_QTY_ABSOLUTE_CAP_OVERRIDE_ENABLED`: when explicitly enabled for Eudy, permits an exploratory exchange minimum only if the existing absolute account-risk cap also approves it; defaults to `false`.
...
- Trend Kalman controls:
  - `MODULE_TREND_KALMAN_ENABLED`
  - `MODULE_TREND_KALMAN_Q_RATIO`
  - `MODULE_TREND_KALMAN_SLOPE_MIN`
  - `MODULE_TREND_KALMAN_BOOST`
...
- `ALLOCATOR_STRONG_TREND_CONFIDENCE_MIN_BY_CONTEXT` (JSON overrides: `SYMBOL:session`, `SYMBOL:*`, `*:session`, `*:*`)
...
- `ALLOCATOR_STRONG_TREND_SOLO_MIN_VOLUME_RATIO_BY_CONTEXT` (JSON overrides: `SYMBOL:session`, `SYMBOL:*`, `*:session`, `*:*`)

...
Validación aislada: `python scripts/test_close_accounting.py` ejecutó 315 pruebas correctamente (una omitida por no disponer del sondeo privado). `python scripts/test_close_accounting.py execution.tests_eudy_recovery risk.tests_performance_report_message execution.tests_risk_policy config.tests_settings` ejecutó 35 pruebas correctamente. Ambas ejecuciones tuvieron cero intentos de red.

La ampliación con `signals.tests` detectó dos fallos preexistentes: `AllocatorRuntimeThresholdCycleTest.test_allocator_cycle_uses_runtime_net_threshold_override` y `AllocatorWeightingTest.test_allocator_does_not_apply_strong_trend_solo_score_mult_when_carry_is_present`. Ambos se reprodujeron sin la corrección en una exportación exacta del commit Eudy original con el mismo entorno aislado. Esta integración no modifica sus señales.

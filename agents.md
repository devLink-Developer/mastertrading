...
- Pol�tica operativa:
  - `microvol` queda exento
  - mantener el gate detr�s de flag para rollback r�pido si empieza a podar winners �tiles
...
### 2026-04-02 update: leverage boost solo para shorts + min signal score subido
- Analisis 14d (113 trades):
  - alloc_long 41 trades, 37% WR, -7.30% PnL
  - alloc_short 72 trades, 75% WR, +25.47% PnL
  - 10x longs: 17 trades, 29% WR, -4.49% → destructor de ganancias
  - 10x shorts: 60 trades, 77% WR, +23.69% → motor principal de beneficio
  - Hipotetico cap longs a 5x: PnL sube de +18.17% a +20.42%
- Cambio de codigo:
  - `config/settings.py`: nuevo setting `CONFIDENCE_LEVERAGE_LONG_ENABLED` (default `false`)
  - `execution/tasks.py`: `_confidence_adjusted_entry_leverage()` ahora recibe `signal_direction`
  - si `direction=long` y setting `false`, retorna base leverage sin boost
  - shorts mantienen capacidad completa de boost a 10x
- Cambio .env en ambos stacks:
  - `CONFIDENCE_LEVERAGE_LONG_ENABLED=false`
  - `EXECUTION_MIN_SIGNAL_SCORE=0.60` (era 0.50)
- Justificacion del score:
  - trades con score < 0.60 en los 14d: 6 trades adicionales cortados, ahorran ~+4.73% PnL
- Cambios previos en esta sesion (ya aplicados antes):
  - `ALLOCATOR_LONG_SCORE_PENALTY=0.85`
  - `WEAK_LONG_BEAR_WEAK_BLOCK_RECOMMENDED_BIASES` expandido a `balanced,short_bias`
  - `WEAK_LONG_BEAR_WEAK_BLOCK_DAILY_REGIMES` expandido a `bear_weak,transition`
  - `WEAK_LONG_BEAR_WEAK_BLOCK_LEAD_STATES` expandido a `transition,bear_confirmed`
  - `SESSION_SCORE_MIN[overlap]` subido de 0.55 a 0.62
...
- Diagnostico prod:
  ...
  - en 48h seguian entrando muchas se�ales `alloc_long/alloc_short`, pero casi nada se convertia en fills
